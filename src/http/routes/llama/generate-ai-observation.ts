...
export async function generateAIObservation(app: FastifyInstance) {
    app.withTypeProvider<ZodTypeProvider>().register(auth).post("/athletes/:athleteId/anamnesis/ai-observation", {
        ...
        return reply.status(201).send({ observation: "O atleta apresenta boa condição física geral, com destaque para a coordenação motora e resistência cardiorrespiratória dentro dos parâmetros esperados para sua faixa etária. Há leve déficit em flexibilidade e força muscular localizada, especialmente em membros inferiores, sugerindo a necessidade de trabalho específico nesses aspectos. Nenhuma limitação grave foi observada, e o acompanhamento contínuo será importante para monitorar e aprimorar seu desenvolvimento físico." });
    })
}